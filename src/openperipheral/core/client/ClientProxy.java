...
public class ClientProxy extends CommonProxy {
	...
	public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		if ((world instanceof WorldClient)) {
			if (ID == OpenPeripheral.Gui.robotEntity.ordinal()) { return new GuiRobotEntity(new ContainerRobot(player.inventory, (IRobot)world.getEntityByID(x))); }
			...
			if (ID == OpenPeripheral.Gui.ticketMachine.ordinal()) {
				...
				return screen;
			} else if (ID == OpenPeripheral.Gui.robot.ordinal()) { return new GuiRobot(new ContainerGeneric(player.inventory, (IInventory)tile, TileEntityRobot.SLOTS), (TileEntityRobot)tile); }
		}
		return null;
	}

...
public class ItemRobot extends Item {
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
		if (!world.isRemote) {
			...
			Vector3 pos = new Vector3(0, 0, 2).apply(new Rotation(radYaw, 0, 1, 0)).add(Vector3.fromEntity(player));
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
		if (stack.hasTagCompound()) {
			...
			if (tag.hasKey("robotId") && tag.hasKey("controllerX") && tag.hasKey("controllerY") && tag.hasKey("controllerZ")) {
				list.add(String.format("Linked to %s, %s, %s", tag.getInteger("controllerX"), tag.getInteger("controllerY"), tag.getInteger("controllerZ")));

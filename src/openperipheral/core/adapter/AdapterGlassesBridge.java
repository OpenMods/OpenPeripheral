...
public class AdapterGlassesBridge implements IPeripheralAdapter {
	...
	@LuaMethod(
			...
			args = { @Arg(
					...
					type = LuaType.NUMBER), @Arg(
					name = "gradient",
					description = "The gradient direction (0 for none, 1 for horizontal, 2 for vertical)",
					type = LuaType.NUMBER) })
	...
	@LuaMethod(
			returnType = LuaType.VOID,
			onTick = false,
			description = "Reset the Guid of the bridge. This will unlink all users")

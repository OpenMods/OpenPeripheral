...
public class DefinitionMethod {
	...
	public DefinitionMethod(Class klazz, JsonNode json) {
		...
		if (json.isNode("argumentCount")) {
			argumentCount = Integer.parseInt(json
					.getNumberValue("argumentCount"));
		}

		if (json.isNode("replacements")) {
			for (JsonField replacementField : json.getNode("replacements")
					.getFieldList()) {
				replacements.put(
						Integer.parseInt(replacementField.getName().getText()),
						replacementField.getValue().getText());
			}
		}
		...
		if (json.isNode("restrictions")) {
			...
			for (JsonField restrictionField : json.getNode("restrictions")
					.getFieldList()) {
				...
				if (paramId != -1) {
					...
					for (JsonField field : fields.getFieldList()) {
						...
						IRestriction restriction = RestrictionFactory
								.createFromJson(field);
		...
		if (callType == CallType.GET_PROPERTY
				|| callType == CallType.SET_PROPERTY) {
			field = ReflectionHelper.getField(klazz, propertyName, obfuscated);
		} else if (callType == CallType.METHOD) {
			method = ReflectionHelper.getMethod(klazz, new String[] { name,
					obfuscated }, argumentCount);
		}
	}
	...
	public Object execute(TileEntity tile, Object[] args)

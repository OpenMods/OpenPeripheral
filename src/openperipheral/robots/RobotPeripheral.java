	...
	public Object[] callMethod(final IComputerAccess computer, ILuaContext context,
		...
		if (method instanceof RobotMethodDeclaration) {
			...
			if (arguments.length < 1 || !(arguments[0] instanceof Double)) {
				throw new Exception("First argument should be the robot ID");
			}
			...
			if (robot == null) {
				throw new Exception("Bad robot ID specified. Are you sure this robot is linked to this controller?");
			}

			final IRobotUpgradeAdapter upgradeInstance = robot.getInstanceForLuaMethod(method.getLuaName());

			if (upgradeInstance == null) {
				throw new Exception("Can not execute method. Do you have the required robot upgrade installed?");
			}
			...
			if (strippedArguments.length != requiredParameters.length){
				throw new Exception(String.format("Invalid number of parameters. You should be passing %s parameters", strippedArguments.length + 1 ));
			}
			
			for (int i = 0; i < strippedArguments.length; i++) {
				if (!requiredParameters[i].type().getJavaType().isAssignableFrom(strippedArguments[i].getClass())) {
					throw new Exception(String.format("Parameter number %s (%s) should be a %s", i+2, requiredParameters[i].name(), requiredParameters[i].type().getName()));
				}
			}

package openperipheral.robots.common.upgrade.lazer;
...
public class InstanceLazersUpgrade implements IRobotUpgradeInstance {
	...
	@LuaMethod
	public boolean fire() {
		return fireLazer(MetaEnergyCell.class, false, 3);
	}

	@LuaMethod
	public boolean fireExplosive() {
		return fireLazer(MetaEnergyCell.class, true, 7);
	}
	
	public boolean fireLazer(Class ammoClass, boolean isExplosive, double heatModifier) {
		...
		for (int i=0; i < inventory.getSizeInventory() - 1; i++) {
			...
			if (slot != null && slot.stackSize > 0 && OpenPeripheral.Items.generic.isA(slot, ammoClass)) {
				cellStack = slot;
				cellIndex = i;
				break;
			}
